    ...
    D3Map.prototype._init = function(options) {
        ...
        $(selector).on("GEOJSON_DONE", function(event, json) {
            ...
            .attr("stroke-width", function(data){
                return (data.properties.type=='city')? 0:1;
            })
            ...
            */
            .on("mouseover", function(data) {
                ...
                if (path.attr("data-selected") != "true") {
                    path.transition().style("fill", mouseOverColor);
                }
            })
            ...
            .on("mouseout", function(data) {
                ...
                if (path.attr("data-selected") != "true") {
                    if (data.properties.type === "city") {
                        path.transition().style("fill", cityColor);
                    }
                    else {
                        path.transition().style("fill", provinceColor);
                    }
                }
                else{
                    if (path.properties.type=="city") path.transition().style("fill", "black");
                    else path.transition().style("fill", selectedColor);
                }
                $('.mapTip').hide();
            })
            /*
            .on("click", function(data) {
                ...
                if (path.attr("data-selected") != "true") {
                    ...
                    if (data.properties.type !== "city") {
                        ...
                        console.log(d3.selectAll("[data-province=" + data.properties.name + "]"));
                        d3.selectAll("[data-province=" + data.properties.name + "]")
                        .attr("data-selected", "true")
                        .transition()
                        .attr("fill", selectedColor)
