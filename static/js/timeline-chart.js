(function (global, factory) {
    ...
    var TimelineChart = function () {
        function TimelineChart(element, data, opts) {
            ...
            var groupWidth = options.hideGroupLabels ? 0 : 180;
            ...
            console.log(colorList,colorList(1));
            ...
            var deleteBtns=svg.selectAll('.deleteBtns').data(data).enter().append("svg:image")
                .attr('class', 'deleteBtns').attr('id',function(d,i){
                    return 'deleteBtns'+i;
                })
                .attr('x', 0).attr('y', function (d, i) {
                    return groupHeight * i;
                })
                .attr('width',groupHeight).attr('height',groupHeight)
                .attr('dx', '0').attr("xlink:href", "../static/icons/minus.svg");
            
            
            //$(".deleteBtns").attr('src',"https://upload.wikimedia.org/wikipedia/commons/b/b0/NewTux.svg")
            //.attr('width',60).attr('height',60);

            if (!options.hideGroupLabels) {
                var tip2 = d3.tip().attr('class', 'Chart-d3-tip2').html(options.label_tip);
                svg.call(tip2);
                ...
                }).attr('dx', '5em').text(function (d) {
                    return d.label;
                }).on('click',function(d){
                     window.open("https://baike.baidu.com/item/"+d.label);
            ...
            if (options.tip) {
                if (d3.tip) {
                    var tip = d3.tip().attr('class', 'Chart-d3-tip').html(options.tip);
                    svg.call(tip);
                    //dots.on('mouseover', tip.show).on('mouseout', tip.hide);
                    dots.on('mouseover',function(_){    
